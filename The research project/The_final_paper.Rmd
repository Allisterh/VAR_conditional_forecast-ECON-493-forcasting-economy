...
#(2023 - 2018)
n.end <- 2019
n_test <- 12 * 3 #the times we need go
n_models <- 4  
h.val <- 1 
...
Can_month_housing_sell.ts <- ts(Can_housing_sell_data.raw$Canada, start = c(2007, 1), end = c(2022, 12), frequency = 12)
 ...
 Bank_rate_ARIMA.st <- get_cansim_vector( Bank_rate_ARIMA_raw, start_time = "2007/2/01", end_time = "2022/12/31")
...
c( Bank_rate_ARIMA_year.st,  Bank_rate_ARIMA_month.st)
 ...
 Bank_rate_ARIMA_stationary <- diff((diff( Bank_rate_ARIMA.ts)))
################
...
Stationary_housing_resale_ARIMA.ts <- diff((Can_month_housing_sell_ARIMA.ts))
...
The_conditional_forecasting_1 <- auto.arima(Housing_sell_Stationary_CPI.ts[4:40, 1], xreg=BankRate[4:40, 1:2], stationary=TRUE, approximation = FALSE, parallel = TRUE, stepwise = FALSE,num.cores = 10)
...
Stationary_Bank_ARIMA_forecasting.ts <- matrix(rep(2, times = 6), ncol = 2)
#6.07866570


autoplot(forecast(The_conditional_forecasting_1, 
                 h = 6, 
                 xreg = Stationary_Bank_ARIMA_forecasting.ts)) +
  theme_fivethirtyeight() +
  xlab("Year") +
  ylab("Percentage") +
  ggtitle("Forecasting of Canadian Housing Resales") +
  labs(subtitle = "If the policy rate stay at 2% current level for 6 months") +
  theme(axis.title = element_text())
