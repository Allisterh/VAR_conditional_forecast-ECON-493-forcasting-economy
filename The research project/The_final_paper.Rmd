...
```{r}
#construed model
#note: first I realize its may help them I realize this is waste of time....
...
#moding time
dall_data_model_1 <- auto.arima(Can_month_housing_sell.ts, approximation = FALSE, parallel = TRUE, stepwise = FALSE,
...
all_data_dummy_1 <- auto.arima(Can_month_housing_sell.ts, approximation = FALSE, parallel = TRUE, stepwise = FALSE,
                               num.cores = 10, start.p = 0, start.q = 0, start.P = 0, 
                               start.Q = 0, max.Q = 5, max.P = 5, max.D = 5,
                               max.d = 5, max.p = 5, max.q = 5)
...
models_table <- data.frame(
  ARIMA = c(as.character(all_data_model_1), 
            ...
            as.character(entil_data_without_covid_shock_1), as.character(entil_data_without_covid_shock_2),
...
x <- all_data_df %>%
  dplyr::select(
   ...
   NHPI = new_housing_price_index_VAR.ts,
   HPI,
   unemployment_rate.ts,
   Bank_rate.ts
  ) %>%
...
#transfer the data into number becasue time series does not
...
# Fit Lasso model with a range of lambda values
...
# Fit Lasso model with the optimal lambda value from cross-validation
...
#The backward search suggest that SCPI, NHPI and HP are variable been picked


```{r}

#set a new data series just for fit the model

#the Var data
Var_all_data <- cbind(stationary_Can_month_housing_sell.ts, Stationary_CPI.ts, HPI, stationary_CA_GDP.ts)
...
The_covid_dummy_VAR.raw <- ts(The_covid_dummy, start = c(2007, 1), end = c(2023, 2), frequency = 12)

The_covid_dummy_VAR.ts <- window(The_covid_dummy_VAR.raw, start= c(2008, 1), end = c(2022, 12))
###
exogen_variable <- cbind(The_covid_dummy_VAR.ts, Bank_rate.ts)

####
VARselect(Var_all_data, exogen = exogen_variable, type="const", lag.max = 24)

var_model <- VAR(Var_all_data, exogen = exogen_variable, p = 3, type="const")
...
housing_gdp_causality <- causality(var_model, cause = "stationary_CA_GDP.ts")$Granger
housing_CPI_causality <- causality(var_model, cause = "Stationary_CPI.ts")$Granger
housing_houing_price_causality <- causality(var_model, cause = "HPI")$Granger
housing_new_houing_price_causality <- causality(var_model, cause = "stationary_Can_month_housing_sell.ts")$Granger
...
Var_all_data_with_exogen <- cbind(Var_all_data, exogen_variable)

# Set the column names for the combined data matrix
colnames(Var_all_data_with_exogen) <- c(colnames(Var_all_data), "Covid_dummy", "Bank_rate")

# Estimate the VAR model without the exogen argument
var_model_with_exogen <- VAR(Var_all_data_with_exogen, p = 3, type="const")

# Perform Granger causality tests
housing_covid_dummy_causality <- causality(var_model_with_exogen, cause = "Covid_dummy")$Granger
housing_prime_rate_causality <- causality(var_model_with_exogen, cause = "Bank_rate")$Granger

# Print the results
print(housing_covid_dummy_causality)
...
#forecasting_Time!
Var_forecasting_one <- forecast(var_model, dumvar = Var_exogen_one , h = 12 )

Var_forecasting_two <- forecast(var_model, dumvar = Var_exogen_two , h = 12)

# Print the results
autoplot(Var_forecasting_one$forecast$Stationary_CPI.ts )
autoplot(Var_forecasting_two$forecast$Stationary_CPI.ts )
...
VARselect(all_dat_fun, exogen = exogen_variable,  type="const", lag.max = 11)
